...
class View
{
    ...
    public function getProduct($mod, $mod1)
    {
        $controller = new Controller();
    ...
    {
        echo '<a href="'. Config::Url .'busket/"><strong>Cart</strong> : <br>
        ...
        Total price : <span>'.$_SESSION['total_price'].'</span></a>';
    }
    ...
    public function showBusket($cart)
    {
        foreach ( $cart as $id => $qty ):
          ...
          $str .= '<td>' . $product[1] . '</td>';
          $str .= '<td>' . $product[2] . '</td>';
          $str .= '<td>' .  number_format($product[5], 2) . 'UAH. </td>';
          $str .= '<td><input type="text" name="' . $id . '"size="2" value="' . $qty . '"maxlength="2"></td>';

          $str .= '<td>' . $product[5] * $qty . 'UAH </td>';
